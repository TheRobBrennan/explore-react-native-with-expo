...
// Simulate a lack of network connectivity when importing @react-native-community/netinfo
...
describe("NetworkAvailable WITHOUT network connectivity", () => {
  afterAll(() => {
    jest.resetModules();
  });
  ...
  test("should render using render from @testing-library/react-native", () => {
    const result = render(<NetworkAvailable />);
    expect(result).toBeDefined();
  });
});
